...
def main():
    ...
    print(
        f"Test Date: {datetime.now(pytz.timezone('Europe/Prague')).strftime('%B %d, %Y %H:%M %Z')}"
    )
    print()
    ...
    print("Weather Conditions (Typical January):")
    ...
    print(
        f"  Actual GHI (with clouds): {result.ghi * (1 - winter_conditions['cloud_cover']/100 * 0.75):.2f} W/m²"
    )
    ...
    times = pd.date_range(
        start=today_start, end=today_end, freq="15min", tz=prague_tz, inclusive="left"
    )
    ...
    if len(production_df) > 0:
        ...
        hourly = (
            production_df.groupby("hour")
            .agg({"power_dc_kw": "mean", "solar_elevation": "mean", "poa_irradiance": "mean"})
            .round(2)
        )
        ...
        for hour, row in hourly.iterrows():
            print(
                f"{hour:02d}:00  {row['power_dc_kw']:>6.2f} kW    {row['solar_elevation']:>6.2f}°       {row['poa_irradiance']:>6.0f} W/m²"
            )
        print()
    else:
